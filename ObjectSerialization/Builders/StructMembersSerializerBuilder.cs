...
namespace ObjectSerialization.Builders
{
    internal class StructMembersSerializerBuilder<T> : SerializerBuilder
    {
        ...
        private static void Build()
        {
            ...
            IOrderedEnumerable<FieldInfo> fields = typeof(T).GetFields(BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic).Where(f => !f.IsInitOnly).OrderBy(p => p.Name);
